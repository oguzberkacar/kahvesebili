...
export default function StationPage() {
  ...
  // Debug & Utils Content (Shared wrapper)
  const debugContent = (
    <>
      <DebugMenu
        isOpen={debugOpen}
        ...
        deviceId={process.env.NEXT_PUBLIC_DEVICE_ID || "unknown"}
        onRefresh={() => reannounce?.()}
      />
      {/* Secret Trigger (Top Left) */}
      <div
        onClick={() => setDebugOpen(true)}
        className="fixed top-0 left-0 w-24 h-24 z-[9999] cursor-pointer opacity-0 hover:opacity-10 active:bg-white/20 transition-colors"
